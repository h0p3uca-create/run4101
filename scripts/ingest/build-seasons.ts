...
async function build() {
  ...
  for (const ver of [...fifa.keys()].sort()) {
    ...
    const clubs = table.map((c, i) => {
      const squad = (squadByClub.get(normClub(c.club)) ?? [])
        .sort((a, b) => b.rating - a.rating).slice(0, 24);
      const str = squadStrength(squad, (10.5 - (i + 1)) * 0.3);
      return { id: normClub(c.club), name: c.club, pos: i + 1, pts: c.pts, gf: c.gf, ga: c.ga, tier: tier(i), attack: str.attack, defense: str.defense, squad };
    });
